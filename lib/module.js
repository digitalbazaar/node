...
function tryPackage(requestPath, exts, isMain) {
  ...
  return tryFile(filename, isMain) || tryExtensions(filename, exts, isMain) ||
...
function tryFile(requestPath, isMain) {
  ...
  if (!isMain) {
    return rc === 0 && toSymbolicPath(requestPath);
  }
...
function toSymbolicPath(requestPath) {
...
Module._findPath = function(request, paths, isMain) {
  ...
  for (var i = 0, PL = paths.length; i < PL; i++) {
    ...
    if (!trailingSlash) {
      ...
      if (rc === 0) {  // File.
        // If running main module
        if (!isMain) {
          filename = toSymbolicPath(basePath);
        } else {
