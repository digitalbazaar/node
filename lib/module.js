...
function tryPackage(requestPath, exts, absolute) {
  var pkg = readPackage(requestPath);

  if (!pkg) return false;

  var filename = path.resolve(requestPath, pkg);
  return tryFile(filename, absolute) ||
         tryExtensions(filename, exts, absolute) ||
...
// resolve to an absolute path if running main module,
// otherwise resolve while keeping symlinks intact.
function tryFile(requestPath, absolute) {
...
function tryExtensions(p, exts, absolute) {
  for (var i = 0, EL = exts.length; i < EL; i++) {
    var filename = tryFile(p + exts[i], absolute);
...
var warned = false;
Module._findPath = function(request, paths, absolute) {
  ...
  for (var i = 0, PL = paths.length; i < PL; i++) {
    ...
    if (!trailingSlash) {
      ...
      if (rc === 0) {  // File.
        ...
        filename = tryPackage(basePath, exts, absolute);
      }

      if (!filename) {
        // try it with each of the extensions
        filename = tryExtensions(basePath, exts, absolute);
      }
    }
    ...
    if (!filename) {
      ...
      filename =
        tryExtensions(path.resolve(basePath, 'index'), exts, absolute);
    }
...
Module._load = function(request, parent, isMain) {
  ...
  if (isMain) {
    var filename = Module._resolveFilename(request, parent, true);
    var absoluteFilename = filename;
  } else {
    var filename = Module._resolveFilename(request, parent, false);
    var absoluteFilename = Module._resolveFilename(request, parent, true);
  }

  var cachedModule = Module._cache[absoluteFilename];
  if (cachedModule) {
...
Module._resolveFilename = function(request, parent, absolute) {
  if (NativeModule.nonInternalExists(request)) {
    return request;
  }
  ...
  var filename = Module._findPath(request, paths, absolute);
  if (!filename) {
